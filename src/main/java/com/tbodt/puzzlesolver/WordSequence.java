...
public final class WordSequence implements Iterable<WordSequence.Word> {
    ...
    public static java.util.function.Function<WordSequence, Stream<WordSequence>> forEachWord(java.util.function.Function<Word, Stream<Word>> function) {
        // This code is complicated, but it simply performs the operation on the first word and discards the rest. It's only temporary.
        return ws -> function.apply(ws.getWords().get(0)).map(w -> new WordSequence(Collections.singletonList(w)));
    }
